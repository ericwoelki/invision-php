...
it('deletes a comment reaction', function (): void {
    MockClient::global([
        DeleteCommentReactionRequest::class => new InvisionFixture('blogs/comments/reactions/delete'),
    ]);

    $comment = $this->invision->blogs()->comments()->reactions()->delete(new DeleteCommentReactionPayload(
        commentId: 2,
        memberId: 2,
    ));
